...
struct POP_EXPORTS GeometricalTransformation
{
    ...
    */
    ...
    static Function scale(const Function & f,const VecN<Function::DIM,F64> & scale,int interpolation=0)
    {
        typename Function::Domain domain (scale*VecN<Function::DIM,F64>(f.getDomain()));
        Function temp(domain);
        VecN<Function::DIM,F64> alpha = VecN<Function::DIM,F64>(1)/scale;
        typename Function::IteratorEDomain it (temp.getIteratorEDomain());
        while(it.next()){
            VecN<Function::DIM,F64> x;
            x=VecN<Function::DIM,F64>(it.x())*alpha;
            if(f.isValid(x))
            {
                if(interpolation==0)
                    temp(it.x())=f(x);
                else
                    temp(it.x())=f.interpolateLinear(x);
            }
        }
        return temp;
    }
    ...
    */
    template<typename Function>
    static Function rotateMultPi_2(const Function & f,int plus_minus=1)throw(pexception)
    {
        Function g(f.getDomain()(1),f.getDomain()(0));
    ...
    */
    template<typename Function>
    static Function rotate(const Function & f,double angle)
    {
        if(Function::DIM==2){
            Mat2x22F64 rot22 = GeometricalTransformation::rotation2D(angle);
            typename Function::IteratorEDomain it = f.getIteratorEDomain();

            Function g(f.getDomain());
    ...
    */
    template<typename Function>
    static Function mirror(const Function & f,int coordinate=0){
     ...
     * \param f input matrix
     * \param boundarycondtion 0=bounded, 1 =periodic
     *
     ...
     */
    template<typename Function>
    static Function translate(const Function & f,const typename Function::E &trans,int boundarycondtion=0)
    {
        Function temp(f.getDomain());
        typename Function::IteratorEDomain it = f.getIteratorEDomain();
        while(it.next())
        {
            typename Function::E z = it.x()+trans;
            if(boundarycondtion==0&&BoundaryConditionBounded::VecNInsideDomain(f.getDomain(),z))
                temp(z)=f(it.x());
            else if(BoundaryConditionPeriodic::VecNInsideDomain(f.getDomain(),z))
                temp(z)=f(it.x());
        }
        return temp;
    }
    ...
    static MatN<DIM,Type> elasticDeformation(const MatN<DIM,Type> & Img,double sigma=10,double alpha=8)
    {
        ...
        ForEachDomain(x,Img){
            for(unsigned int i=0;i<DIM;i++)
                dist(i)(x)=d.randomVariable();
        }
        for(unsigned int i=0;i<DIM;i++)
            dist(i) = ProcessingAdvanced::smoothGaussian(dist(i),sigma,sigma*2.5,dist(i).getIteratorEDomain());


        Mat2F64::IteratorEDomain itr = dist(0).getIteratorEDomain();
        double standart_deviation = AnalysisAdvanced::standardDeviationValue(dist(0),itr);
        ForEachDomain(x,Img){
            for(unsigned int i=0;i<DIM;i++)
                dist(i)(x)*=alpha/standart_deviation;
        }
        MatN<DIM,Type> mdist(Img.getDomain());
        ForEachDomain(x,mdist){
            VecN<DIM,F64> xx(x);
            for(unsigned int i=0;i<DIM;i++)
                xx(i)+=dist(i)(x);
            if(Img.isValid(xx)){
                mdist(x)=Img.interpolateLinear(xx);
            }
        }
        return mdist;
    }
    ...
    */
    ...
    static MatN<2,Type> transformHomogeneous2D(const pop::Mat2x33F64 & mproj,const MatN<2,Type> & f,bool automaticsize=false)
    {
        ...
        while(it.next()){
            ...
            if(f.isValid(x))
                gtransform(it.x())=f.interpolateLinear(x);
        }
        return gtransform;
    }
    /*!
