...
#include"core/data/notstable/SearchStructure.h"
...
public:
    ...
    */
    ...
    static Vec<KeyPoint<Function::DIM> > keyPointHarris(const Function & img,double sigma = 2,double kappa=0.20)
    {
        typedef typename FunctionTypeTraitsSubstituteF<Function,F64>::Result FunctionFloat;
        FunctionFloat imgf(img);
        imgf = Processing::greylevelRange(imgf,0,1);
        FunctorPDE::HessianMatrix<> func_hessian;
        typename FunctionFloat::IteratorEDomain itdomain(imgf.getIteratorEDomain());


        typedef Mat2x<F64,Function::DIM,Function::DIM> MatrixDim;
        typedef typename FunctionTypeTraitsSubstituteF<FunctionFloat,MatrixDim >::Result FunctionMatrix;

        FunctionMatrix img_hessian = func_hessian.iterate(imgf,itdomain);
        FunctorPDE::GaussianKernel<FunctionMatrix> func_gaussian(img_hessian,sigma,3*sigma,true);

        itdomain.init();
        img_hessian = func_gaussian.iterate(img_hessian,itdomain);
        itdomain.init();
        while(itdomain.next()){
            Mat2x22F64 & m=img_hessian(itdomain.x());
            imgf(itdomain.x())= m.determinant()-kappa*m.trace()*m.trace();
        }
        typename FunctionFloat::IteratorENeighborhood itneigh(imgf.getIteratorENeighborhood(1,0));
        itdomain.init();
        Vec<KeyPoint<Function::DIM> > v_maxima;
        while(itdomain.next()){
            if(imgf(itdomain.x())>0.00001){
                typename FunctionFloat::F value = imgf(itdomain.x());
                bool maxima=true;
                ...
                if(maxima==true){
                    KeyPoint<Function::DIM> keypoint(itdomain.x());
    ...
    */
    ...
    static Pyramid<DIM,F64 > pyramidGaussian(const MatN<DIM,Type> & img,  double sigma=1.6,int number_octave=-1,int number_layers_per_octave=3,int number_extra_layer_per_octave=3)
    {
        ...
        FunctorPDE::GaussianKernel<MatN<DIM,F64> > d1(I,sigma,std::ceil(2.5*sigma),true);
        typename MatN<DIM,F64>::IteratorEDomain it= img.getIteratorEDomain();
        I = d1.iterate(I,it);
        ...
        for(int i=0;i<number_octave;i++){
            ...
            for(int j=1;j< number_layers_per_octave + number_extra_layer_per_octave;j++){
                ...
                FunctorPDE::GaussianKernel<MatN<DIM,F64> > d(temp,sig[j],maximum(2.,std::ceil(sig[j]*2.5)),true);
                temp = d.iterate(temp,it);
    ...
    */
    ...
    static VecN<2,Type> orientationPrincipalHistogram(const Vec<VecN<2,Type> > & v_orientions, const Vec<Type > & weight,int nbr_angle=36)
    {
        ...
        for(unsigned int i=0;i<v_orientions.size();i++){
            ...
            std::vector<F64>::const_iterator  low=lower_bound (angles.begin(), angles.end(),angle );
    ...
    static VecN<Function::DIM,F64>   orientationMeanDoubleWeighByGradienMagnitudeAndGaussianCircularWindow(const Function& f, IteratorNeigh& itn,double sigma)
    {
    ...
    template<typename Function,typename IteratorNeigh>
    static Mat2F64   pieChartInPieChartDoubleWeighByGradienMagnitudeAndGaussianCircularWindow(const Function& f, IteratorNeigh& itn,VecN<Function::DIM,F64> &orientation,int nbr_orientation, double sigma)
    {
        FunctorPDE::Gradient<> gradient;
