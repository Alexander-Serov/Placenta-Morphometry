#include "CharacteristicCluster.h"
...
double CharacteristicClusterDistanceWidthInterval::operator ()(const CharacteristicCluster& a,const CharacteristicCluster& b){
    if(a.size()(1)!=0&&b.size()(1)!=0)
        return std::abs(a.center()(1)-b.center()(1))/std::min(a.size()(1),b.size()(1));
    else
        return std::numeric_limits<double>::max();
}
...
double CharacteristicClusterDistanceHeightInterval::operator ()(const CharacteristicCluster& a,const CharacteristicCluster& b){
    if(a.size()(0)!=0&&b.size()(0)!=0)
        return std::abs(a.center()(0)-b.center()(0))/std::min(a.size()(0),b.size()(0));
    else
        return std::numeric_limits<double>::max();
}
...
//        for(int j=0;j<v_v_neigh[i].size();j++){
...
//            Draw::line(graph,v_cluster[i].center(),v_cluster[index]._barycentre,RGBUI8(255,0,0),3);
//        }
//    }
//    graph.display();
