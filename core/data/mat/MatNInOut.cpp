#ifndef NOCIMG
#include"core/dependency/ConvertorCImg.h"
#endif
#include"core/dependency/lodepng.h"
#include"core/dependency/bipmap.h"
#include"core/dependency/jpgd.h"
#include"core/dependency/jpge.h"
...
void  MatNInOut::_loadPNG( MatN<2, UI8 > &img, const char * filename)throw(pexception)
{
    ...
    img.clear();
    lodepng::decode(img, width, height, filename,LCT_GREY,8);
    if(img.size()==0)
...
void  MatNInOut::_saveJPG(const MatN<2, RGBUI8 > &img, const char * filename){
    jpge::compress_image_to_jpeg_file(filename,img.getDomain()(1),img.getDomain()(0),3,(unsigned char*)img.data());
}

void  MatNInOut::_saveJPG(const MatN<2, UI8 > &img, const char * filename){
    jpge::compress_image_to_jpeg_file(filename,img.getDomain()(1),img.getDomain()(0),1,img.data());
}
