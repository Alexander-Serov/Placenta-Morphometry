...
void DrawGLScene()
{
    ...
    VecN<3,F32> _min = 100000;
    for(int i =0;i<(int)op->vfigure.size();i++)
    {
        _max = maximum(_max, op->vfigure[i]->getMax());
        _min = minimum(_min, op->vfigure[i]->getMin());
    }
    ...
    //    glLoadIdentity();
    //  glTranslatef(16,16,-20);
    // Move Right 3 Units, and back into the screen 7

    //    glRotatef(rquad,1.0f,1.0f,1.0f);        // Rotate The Cube On X, Y, and Z
    glScalef(scale*scale_global, scale*scale_global, scale*scale_global);
    //    glScalef(1./56, 1./56, 1./56);
    glRotated(-90, 1.0, 0.0, 0.0);
    ...
    for(int i =0;i<(int)op->vfigure.size();i++)
    {
        if(i==0)
        {
            op->vfigure[i]->callBeginMode();
        }
        else if(typeid(*(op->vfigure[i]))!=typeid(*(op->vfigure[i-1])))
        {
            op->vfigure[i]->callBeginMode();
        }
        op->vfigure[i]->draw();
        if(i==(int)op->vfigure.size()-1)
        {
            op->vfigure[i]->callEndMode();
        }
        else if(typeid(*(op->vfigure[i]))!=typeid(*(op->vfigure[i+1])))
        {
            op->vfigure[i]->callEndMode();
...
int DrawGLScene()                                    // Here's Where We Do All The Drawing
{
    ...
    for(int i =0;i<(int)op->vfigure.size();i++)
    {
        VecN<3,F32> v =op->vfigure[i]->getMax();
        for(int j =0;j<3;j++){
            if(v(j)>_max(j))
                _max(j)=v(j);
        }
        v =op->vfigure[i]->getMin();
        for(int j =0;j<3;j++)
            if(v(j)<_min(j))
                _min(j)=v(j);
...
void Scene3d::clear(){
    for(int i =0;i<(int)vfigure.size();i++){
        delete vfigure[i];
    }
    vfigure.clear();
}
...
Scene3d& Scene3d::operator =(const Scene3d& g){
    ...
    for(int i=0; i <(int)vfigure.size();i++)
    {
        delete this->vfigure[i];
    }
    this->vfigure.clear();
    for(int i =0;i<(int)g.vfigure.size();i++)
    {
        this->vfigure.push_back(g.vfigure[i]->clone());
    }
    return *this;
}
...
void Scene3d::setColorAllGeometricalFigure(const RGBUI8 & value){
    for(unsigned int i=0;i<this->vfigure.size();i++){
        GeometricalFigure* figure = this->vfigure[i];
        figure->setRGB(value);
    }
}
...
void Scene3d::setTransparencyAllGeometricalFigure( UI8  value){
    for(unsigned int i=0;i<this->vfigure.size();i++)
        this->vfigure[i]->setTransparent(value);
}
